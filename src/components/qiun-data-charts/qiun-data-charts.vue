...
<template>
    <view class="chartsview" :id="'ChartBoxId'+cid">
        ...
        <!-- #ifdef MP-ALIPAY -->
        <block v-if="ontouch">
            <canvas :id="cid" :canvasId="cid" :width="cWidth * pixel" :height="cHeight * pixel" :style="{ width: cWidth + 'px', height: cHeight + 'px', background: background }" :disable-scroll="disScroll" @tap="_tap" @touchstart="_touchStart" @touchmove="_touchMove" @touchend="_touchEnd" @error="_error" v-if="showchart" />
        </block>
        <block v-if="!ontouch">
            <canvas :id="cid" :canvasId="cid" :width="cWidth * pixel" :height="cHeight * pixel" :style="{ width: cWidth + 'px', height: cHeight + 'px', background: background }" :disable-scroll="disScroll" @tap="_tap" @error="_error" v-if="showchart" />
        </block>
        <!-- #endif -->
        <!-- 其他小程序通过vue渲染图表 -->
        ...
        <block v-if="type2d">
            <view v-if="ontouch" @tap="_tap">
                <canvas :id="cid" :canvasId="cid" :style="{ width: cWidth + 'px', height: cHeight + 'px', background: background }" type="2d" :disable-scroll="disScroll" @touchstart="_touchStart" @touchmove="_touchMove" @touchend="_touchEnd" @error="_error" v-show="showchart" />
            </view>
            <view v-if="!ontouch" @tap="_tap">
                <canvas :id="cid" :canvasId="cid" :style="{ width: cWidth + 'px', height: cHeight + 'px', background: background }" type="2d" :disable-scroll="disScroll" @error="_error" v-show="showchart" />
            </view>
        </block>
        <block v-if="!type2d">
            <view v-if="ontouch" @tap="_tap">
                <canvas :id="cid" :canvasId="cid" :style="{ width: cWidth + 'px', height: cHeight + 'px', background: background }" @touchstart="_touchStart" @touchmove="_touchMove" @touchend="_touchEnd" :disable-scroll="disScroll" @error="_error" v-if="showchart" />
            </view>
            <view v-if="!ontouch">
                <canvas :id="cid" :canvasId="cid" :style="{ width: cWidth + 'px', height: cHeight + 'px', background: background }" :disable-scroll="disScroll" @tap="_tap" @error="_error" v-if="showchart" />
            </view>
        </block>
        <!-- #endif -->
    </view>
...
function formatterAssign(args, formatter) {
    for (let key in args) {
        if (typeof args[key] === "object") {
            if (args[key] && Object.keys(args[key]).length < 15) {
...
export default {
    ...
    created() {
        ...
        if (this.canvas2d === false) {
            ...
            if (this.canvasId === "uchartsid" || this.canvasId == "") {
                console.log(
                    "[uCharts]:开启canvas2d模式，必须指定canvasId，否则会出现偶尔获取不到dom节点的问题！"
                );
            }
        }
        // #endif
        ...
        checkData(anyData) {
            ...
            if (this.echarts === true) {
                if (this.type && cfe.type.includes(this.type)) {
                    cfe.option[cid] = deepCloneAssign(
                        {},
                        cfe[this.type],
                        this.eopts
                    );
                ...
                if (this.type && cfu.type.includes(this.type)) {
                    cfu.option[cid] = deepCloneAssign(
                        {},
                        cfu[this.type],
                        this.opts
                    );
            ...
            if (newData.series !== undefined && newData.series.length > 0) {
                ...
                if (this.echarts === true) {
                    if (
                        cfe.option[cid].xAxis &&
                        cfe.option[cid].xAxis.type &&
                        cfe.option[cid].xAxis.type === "category"
                    ) {
                        cfe.option[cid].xAxis.data = newData.categories;
                    }
                    if (
                        cfe.option[cid].yAxis &&
                        cfe.option[cid].yAxis.type &&
                        cfe.option[cid].yAxis.type === "category"
                    ) {
                        cfe.option[cid].yAxis.data = newData.categories;
                    }
                    cfe.option[cid].series = [];
                    for (var i = 0; i < newData.series.length; i++) {
                        cfe.option[cid].seriesTemplate = cfe.option[cid]
                            .seriesTemplate
                            ? cfe.option[cid].seriesTemplate
                            : {};
                        let Template = deepCloneAssign(
                            {},
                            cfe.option[cid].seriesTemplate,
                            newData.series[i]
                        );
            ...
            let chartdom = uni
                ...
                .boundingClientRect((data) => {
                    ...
                    if (data.width > 0 && data.height > 0) {
                        if (
                            data.width !== this.cWidth ||
                            data.height !== this.cHeight
                        ) {
                            this.checkData(this.drawData);
                        }
                    }
                    ...
                    if (data.width > 0 && data.height > 0) {
                        ...
                        if (this.inH5 || this.inApp) {
                            if (this.echarts == true) {
                                ...
                                cfe.option[cid].tooltipFormat =
                                    this.tooltipFormat;
                                cfe.option[cid].tooltipCustom =
                                    this.tooltipCustom;
                                cfe.option[cid].lastDrawTime =
                                    this.lastDrawTime;
                                cfe.option[cid].rotateLock =
                                    cfe.option[cid].rotate;
                                this.echartsOpts = deepCloneAssign(
                                    {},
                                    cfe.option[cid]
                                );
                            } else {
                                cfu.option[cid].rotateLock =
                                    cfu.option[cid].rotate;
                                this.mixinDatacomLoading = false;
                                this.showchart = true;
                                this.uchartsOpts = deepCloneAssign(
                                    {},
                                    cfu.option[cid]
                                );
                            }
                            ...
                            cfu.option[cid] = formatterAssign(
                                cfu.option[cid],
                                cfu.formatter
                            );
                            this.mixinDatacomErrorMessage = null;
                            ...
                            this.$nextTick(() => {
                                if (this.type2d === true) {
                                    ...
                                    query
                                        ...
                                        .exec((res) => {
                                            if (res[0]) {
                                                ...
                                                canvas.width =
                                                    data.width * this.pixel;
                                                canvas.height =
                                                    data.height * this.pixel;
                                                canvas._width =
                                                    data.width * this.pixel;
                                                canvas._height =
                                                    data.height * this.pixel;
                                    ...
                                    cfu.option[cid].context =
                                        uni.createCanvasContext(cid, this);
                                    this._newChart(cid);
                                }
                            });
                        }
                    ...
                    .boundingClientRect((data) => {
                        ...
                        if (this.inAli) {
                            e.changedTouches.unshift({
                                x: e.detail.pageX - data.left,
                                y: e.detail.pageY - data.top,
                            });
                        } else {
                            e.changedTouches.unshift({
                        ...
                        if (move) {
                            ...
                            currentIndex =
                                cfu.instance[cid].getCurrentDataIndex(e);
                            legendIndex =
                                cfu.instance[cid].getLegendDataIndex(e);
